...
namespace assembler {
	...
	std::ptrdiff_t resolve_operand(operand op, register_file& registers) {
	...
	std::string print_register(reg r) {
	...
	std::string print_operand(operand op) {
		if(std::holds_alternative<reg>(op)) {
	...
	{
		inc(reg op1_) noexcept : unary_instruction("inc", op1_) {
		}
		...
		std::string emit_code() override {
			return "++" + print_register(op1) + ";";
		}
	};

	struct dec : unary_instruction
	{
		dec(reg op1_) noexcept : unary_instruction("dec", op1_) {
		}
	...
	instruction_ptr parse_cpy(const std::string& ins) {
	...
	instruction_ptr parse_dec(const std::string& ins) {
		reg r = parse_reg(ins);
	...
	void virtualize_jumps(program& insns) {
	...
	void devirtualize_jumps(program& insns) {
		for(std::size_t i = 0; i < insns.size(); ++i) {
	...
	void print_asm(const program& p) {
